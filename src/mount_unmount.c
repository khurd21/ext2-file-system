...
int mount(const char *filesys, const char *mount_point)
{
    ...
    // 6. Record new dev and filesys name in the MOUNT table entry, store its
    // ninodes, nblocks, bmap, imap and inodes start block, etc. for quick access
    MTABLE* mtable_ptr = &mtable[my_mount_point];
    mtable_ptr->dev = fd;

    strcpy(mtable_ptr->dev_name, filesys);
    strcpy(mtable_ptr->mnt_name, mount_point);

    mtable_ptr->ninodes = sp->s_inodes_count;
    mtable_ptr->nblocks = sp->s_blocks_count;
    mtable_ptr->mnt_dir_ptr = mip;
