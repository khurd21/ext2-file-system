...
int rmdir(const char *pathname)
{
    ...
    // Issue previously: forcing pathname a const type, I found
    // we were accidentally modifying pathname, which caused problems
    // if we were deleting a dir not in cwd

    char temp1[128];
    char temp2[128];
    strcpy(temp1, pathname);
    strcpy(temp2, pathname);
    char *dname = dirname(temp1);
    char *bname = basename(temp2);
...
int rm_child(MINODE *pmip, char *name)
{
    ...
    */
    ...
    char buf[BLKSIZE];
    char potential_name[BLKSIZE / sizeof(int)];
    ...
    for (int i = 0; i < 12; ++i)
    {
        ...
        {
            // Copy the dp->name to a temp char to not risk overwriting it
            strncpy(potential_name, dp->name, dp->name_len);
            potential_name[dp->name_len] = '\0';
            const int len = strlen(potential_name);
            printf("Potential name: %s\n", potential_name);

            // If name not found, continue search
            if (strncmp(potential_name, name, len) != 0)
            {
                // printf("Potential name: %s\n", potential_name);
                // lastdp = dp;
                prevdp = dp;
            ...
            printf("Found name: %s\n", potential_name);
            // Node found
            // Exclusive case: first and only entry in a data block
            if (cp == buf && (dp->rec_len + cp) == (buf + BLKSIZE))
            {
            ...
            else if (cp + dp->rec_len >= BLKSIZE + buf)
            {
                prevdp->rec_len += dp->rec_len;
                put_block(pmip->dev, pmip->INODE.i_block[i], buf);
            }

            // Middle entry in the block
            else
            {
                DIR *finaldp = (DIR *)buf;
                char *finalcp = buf;
